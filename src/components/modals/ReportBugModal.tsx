...
import { reportBugAndUpdate } from "@/lib/slice/StudentSlice";
...
const ReportBugModal = ({ open, setOpen }: Props) => {
	...
	const sendMessage = async () => {
		// create the data to send with the function
		const data: any = {
			title,
			message,
			type,
		...
		setStatus(true);
		// call the function to submit the bug/update report
		const res = await dispatch(reportBugAndUpdate(data)).unwrap();
		setStatus(false);
		setOpen(false);
		toast(res?.msg);
	};
					...
					<div>
						<div className="flex justify-center items-center gap-8 font-inter text-xs text-slate-900 font-semibold">
							<button
								...
								className={`${
									type === "bug" ? "bg-green-600" : "bg-green-400"
								} rounded-full px-4 py-2 cursor-pointer duration-500 hover:bg-green-600`}
							>
								Report a bug
							</button>
							<button
								onClick={() => {
									setTitle("Suggest an update");
									setType("update");
								}}
								className={`${
									type === "update" ? "bg-green-600" : "bg-green-400"
								} rounded-full px-4 py-2 cursor-pointer duration-500 hover:bg-green-600`}
							>
								Suggest an update
							</button>
